...
describe('ProductList', () => {
  ...
  it('商品一覧のタイトルが表示される', () => {
    ...
    expect(screen.getByRole('heading', { name: '商品一覧', level: 1 })).toBeInTheDocument();
  });
  ...
  it('空の商品リストが渡された場合、商品カードは表示されない', () => {
    ...
    expect(screen.queryByRole('button', { name: '購入数を選択' })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: '在庫切れ' })).not.toBeInTheDocument();
