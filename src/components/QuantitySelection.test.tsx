...
describe("QuantitySelection", () => {
  ...
  it("ページタイトル「購入数の選択」が表示される", () => {
    render(
      <QuantitySelection onComplete={mockOnComplete} onCancel={mockOnCancel} />
    );

    expect(
      screen.getByRole("heading", { name: "購入数の選択", level: 1 })
    ).toBeInTheDocument();
  });
  ...
  it("選択された商品の情報が表示される", () => {
    ...
    // 商品名（h2）
    expect(
      screen.getByRole("heading", { name: product.name, level: 2 })
    ).toBeInTheDocument();
  ...
  it("カートに既に同じ商品がある場合、カート内の状態が表示される", () => {
    ...
    expect(
      screen.getByText(/この商品は既に.*2個.*カートに入っています/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/追加すると合計3個になります/i)
    ).toBeInTheDocument();
  });
  ...
  it("最大数量のヘルプテキストが表示される", () => {
    ...
    const product = mockProducts[0];
    expect(
      screen.getByText(`最大 ${product.stock}個まで選択できます`)
    ).toBeInTheDocument();
  });
});
