...
describe('Cart', () => {
  ...
  it('カート内の商品が正しく表示される', () => {
    ...
    cartItems.forEach(({ product, quantity }) => {
      ...
      // 単価表示
      expect(screen.getByText(`¥${product.price.toLocaleString()} / 個`)).toBeInTheDocument();
  ...
  it('ヘッダーに「ショッピングカート」と表示される', () => {
    const cartItems = [{ product: mockProducts[0], quantity: 1, addedAt: Date.now() }];
  ...
  it('合計金額と商品数が正しく表示される', () => {
    ...
    expect(screen.getByText(`合計金額 ¥${total.toLocaleString()}`)).toBeInTheDocument();
  });
  ...
  it('数量が在庫数と同じ時、増やすボタンが無効化される', () => {
    ...
    const cartItems = [{ product, quantity: product.stock, addedAt: Date.now() }];
